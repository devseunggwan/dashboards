...
def sidebar():
    with st.sidebar:
        ...
        st.selectbox("Selected Network", st.session_state.reservoir["networks"], key="selected_network")
        st.text_input("Collection ID", key="input_collection_id")

        with st.expander("LLM Model Options"):
            st.selectbox("Selected Model", st.session_state.llm_models, key="selected_model")
            st.slider("NFT Images", 1, 20, 10, 1, key="image_count")
